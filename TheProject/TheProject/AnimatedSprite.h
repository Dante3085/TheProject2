...
namespace TheProject
{
	...
	public:
		...
		/**
		 * \brief Returns if all State-Transition-Conditions of this AnimatedSprite(Entity) for the given State are satisfied
		 * \return If all State-Transition-Conditions of this AnimatedSprite(Entity) for the given State are satisfied
		 */
		bool checkConditions(EState state) override;

		void addCondition(EState state, bool(*condition)()) override;
	...
	private:
		...
		/**
		 * \brief Holds this AnimatedSprite's State-Transition-Conditions
		 */
		std::map<EState, std::vector<bool(*)()>> m_conditions;
