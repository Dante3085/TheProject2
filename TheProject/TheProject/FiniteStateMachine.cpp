...
namespace TheProject
{
	...
	FiniteStateMachine::FiniteStateMachine()
		: m_states{ new std::map<EState, State*> }, m_startState{ debugging }, m_endState{ none }, m_currentState(none)
	{
	...
	bool FiniteStateMachine::change(EState state)
	{
		...
		// Check if Entities of the current State allow State-Transition to given State
		bool condition = true;
		std::vector<Entity*>* entites = m_states->at(m_currentState)->getEntities();
		for (auto& entity : *entites)
			condition = condition && entity->checkConditions(state);

		if (!condition)
		{
			std::cout << "@FiniteStateMachine.change(): CurrentState \"" << m_states->at(m_currentState)->getName() <<
				" does currently not allow transitioning to passed state. Not all conditions have been met" << std::endl;
		}
