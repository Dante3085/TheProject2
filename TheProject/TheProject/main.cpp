...
bool atPosition(AnimatedSprite& animSprite)
{
	Vector2f v{ 0, 0 };
	return animSprite.getPosition() == v;
}
...
int main()
{
	...
	// phantom->addCondition(none, atPosition(*phantom));
	...
	std::vector<Entity*>* entities = new std::vector<Entity*>{ fire, phantom, colMan, vortex, demon, loadingSprite, loadingSprite2,
	bubbles, swordsman};
	std::vector<EState>* next = new std::vector<EState>{ none };

	std::map<EState, State*>* states = new std::map<EState, State*>
	{ 
		{debugging, new SceneState{"Debugging", entities, next, "Ressources/preview-old-dark-castle-interior-tileset.png", (int)window.getSize().x, (int)window.getSize().y}}, 
	};

	FiniteStateMachine fsm{ states, debugging, none };
